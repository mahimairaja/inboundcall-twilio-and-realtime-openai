...
def print_conversation_message(response_type, message):
    """Print formatted conversation messages."""
        ...
        async def send_to_twilio():
            ...
            try:
                async for openai_message in openai_ws:
                    ...
                    # Print transcribed user input
                    if response.get('type') == 'input_audio_buffer.committed':
                        if response.get('item', {}).get('content', []):
                            for content in response['item']['content']:
                                if content.get('type') == 'text':
                                    print_conversation_message('user', content['text'])
                                ...
                                if item.get('role') == 'assistant' and item.get('content'):
                                    for content in item['content']:
                                        if content.get('type') == 'audio':
                                            print_conversation_message('assistant', content['transcript'])
